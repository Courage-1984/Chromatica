...
class AnnIndex:
    # ... (rest of AnnIndex class is unchanged)
    def __init__(
        self,
        dimension: int,
...
class MetadataStore:
    ...
    # --- NEW METHOD FOR DE-DUPLICATION ---
    def check_existence(self, image_ids: List[str]) -> Set[str]:
        """
        Queries the DuckDB metadata store to check which of the given image IDs
        already exist in the 'image_metadata' table.

        This is used in append mode to prevent indexing duplicate images.

        Args:
            image_ids: A list of image ID strings (e.g., filename stems).

        Returns:
            A set of image IDs that were found in the database.
        """
        if not image_ids:
            return set()
        ...
        try:
            # Use the connection to execute the query with the list of IDs
            result = self.connection.execute(query_sql, image_ids).fetchall()

            # The result is a list of tuples, e.g., [('id1',), ('id3',)]
            existing_ids = {row[0] for row in result}

            return existing_ids

        except Exception as e:
            logger.error(f"Error checking image existence in DuckDB: {e}")
            # Return an empty set to allow indexing to proceed, relying on the
            # PRIMARY KEY constraint to catch duplicates on insertion if possible.
            return set()

    # ------------------------------------

    def _get_histogram_from_cache(self, image_id: str) -> Optional[np.ndarray]:
        # ... (rest of MetadataStore is unchanged)
        """
