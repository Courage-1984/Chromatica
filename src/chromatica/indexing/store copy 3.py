...
def hellinger_transform(vectors: np.ndarray) -> np.ndarray:
    ...
    """
    # Clip values to ensure non-negativity before taking the square root
    # Using np.maximum for performance over clipping on creation
    return np.sqrt(np.maximum(0.0, vectors))
...
class MetadataStore:
    ...
    def __init__(self, db_path: Union[str, Path], table_name: str = "images"):
        self.db_path = Path(db_path)
        self.table_name = table_name
        self.connection = None
        self._lock = threading.Lock()
        self._connect()
        # Clear the cache whenever the connection might be reset or database modified
